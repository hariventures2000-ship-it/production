...
export default function NotificationBell({
  ...
  defaultRole = "ROLE_MANAGING_DIRECTOR",
  defaultUserId = "md",
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Also pass context headers in case gateway filter is disabled but backend expects them
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // Optional: Simple browser desktop notification or alert
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-0.5">
                      ...
                      {!notif.isRead && (
                        <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full flex-shrink-0 mt-1" />
                      )}
                    </div>
